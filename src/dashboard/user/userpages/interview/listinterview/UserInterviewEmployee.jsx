  ...
  const fetchdata = async () => {
    try {
      const res = await axiosclient.get(
        `/shopee-career/get-interview-by-employer/${jwt.id}?page=${page}&size=5`
      );
              ...
              <tr key={item.interviewID}>
                ...
                <td>
                  ...
                  <button className="action-btn status-btn">
                    <NavLink
                      className=""
                      to={`/user/user-detail-interview-employee/${item.interviewID}`}
                    >
                      <i
...
export default UserInterviewEmployee;
