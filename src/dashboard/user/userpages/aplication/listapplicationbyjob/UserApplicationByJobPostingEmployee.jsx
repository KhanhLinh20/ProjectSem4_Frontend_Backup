...
const UserApplicationByJobPostingEmployee = () => {
  ...
  const fetchdata = async () => {
    try {
      const res = await axiosclient.get(
        `/shopee-career/list-application-by-jobposting-employee/${id}/${jwt.id}?page=${page}&size=5`
      );
  ...
  return (
    <div>
      <div className="admin-list">
        <h2 className="my-5">Applications By JobPosting</h2>
              ...
              <tr key={item.applicationID}>
                ...
                <td>
                  <button className="action-btn status-btn">
                    <NavLink
                      className=""
                      to={`/user/user-detail-application-by-jobposting-employee/${item.applicationID}`}
                    >
                      <i
...
export default UserApplicationByJobPostingEmployee;
