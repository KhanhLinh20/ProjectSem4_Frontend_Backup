...
const UserApplicationRejectEmployee = () => {
  ...
  const fetchdata = async () => {
    try {
      const res = await axiosclient.get(
        `/shopee-career/list-application-by-employer-interview-fail/${jwt.id}?page=${page}&size=5`
      );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      ...
      500,
      []
    ); // Thời gian chờ cho loading, có thể điều chỉnh
                ...
                <tr key={item.applicationID}>
                  ...
                  <td>
                    <button className="action-btn status-btn">
                      <NavLink
                        className=""
                        to={`/user/user-detail-application-reject-employee/${item.applicationID}`}
                      >
                        <i
...
export default UserApplicationRejectEmployee;
